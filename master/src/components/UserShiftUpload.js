    ...
    for (let i = 0; i < totalRecords; i += batchSize) {
      ...
      for (let j = i; j < Math.min(i + batchSize, totalRecords); j++) {
        shiftsBatch.push({
          "userid": data[j].col0,
          "STAGE_NAME": data[j].col2,
          "Skill_Description": data[j].col3,
          "Skill_Rating": data[j].col4
        });

        shiftsBatch.push({
          "userid": data[j].col0,
          "STAGE_NAME": data[j].col5,
          "Skill_Description": data[j].col6,
          "Skill_Rating": data[j].col7
        });
      }
      ...
      try {
        const response = await axios.post('http://localhost:5000/api/saveUserSkills', jsonstring, {
          headers: {
            'Content-Type': 'application/json'
          }
        });
        console.log(response.data);
        setMessage('Skill Batches Saving..');
      } catch (error) {
